...
nPoly = 7700; (* Total number of fractures you would like to have 
...
nShape = {3,0,0,0}; (* {ellipses, rectangles, user-ellipses, user-recantles}
...
enumPoints = {15, 15, 15}; (* Number of vertices used in creating each elliptical 
		     fracture family. Number of elements must match number 
		     of ellipse families (first number in nShape) *)
						
etheta = {Pi/2, 2*Pi/2.66, 2*Pi};    (* Ellipse fracture orientation.
                     The angle the normal vector makes with the z-axis *)

ephi = {0, 0, Pi/2}; (* Ellipse fracture orientation.
		The angle the projection of the normal onto the x-y plane
		makes with the x-axis *)

ekappa = {21.7, 21.5, 8.2};  (* Parameter for the fisher distribnShaprutions. The bigger, the more 
...
ellmean = {8.0, 9.0};  (* Mean value for Exponential  Distribution.   *)     

(*===========================================================================*)
(* Options Specific For Constant Size of ellipses (edistr=4): *)

econst = {5, 5};  (* Constant radius for all the ellipses in the DFN    *)   
...
emin = {15, 15, 15}; (* Minimum radius for each ellipse family. 
		     For power law distributions. *)

emax = {560, 560, 560};   (* Maximum radius for each ellipse family.
		     For power law distributions. *)
					
ealpha = {2.5, 2.7,2.38}; (* Alpha. Used in truncated power-law 
			distribution calculation*)

epowerLawReject={100, 100, 100};   (* Maximum number of rejections of one fracture of
...
rdistr={2, 4};   (*  Rectangle statistical distribution options:
		1 - lognormal distribution
		2 - truncated power law distribution  
		3 - exponential distribution
		4 - constant *)
...
(* Mandatory Parameters if using rectangles with power-law distribution *)
 ...
 rmax = { 15, 15};     (* Maximum radius for each rectangle family.
			For power law distributions. *)
...
rconst = {30,30};  (* Constant length for all the rectangles in the DFN    *)   
...
apertureFromTransmissivity={1.6*10^-10, 0.8}; 
